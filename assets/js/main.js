...
$(document).ready(function () {
  ...
  $('.datepicker').datepicker($.datepicker.regional[ language ],{ dateFormat: 'yy-mm-dd' });
  ...
  $(document).on('click', '#send_form', function(event){
    ...
    var validator = form.validate({
      lang: language,  // or whatever language option you have.
...
function submit_contact_form(form, button)
{
  var elements = form.find('.for_send');
  var data = {};
  ...
  $.ajax({
    type: 'POST',
    url: base_url + 'index/ajax_form_process',
    data: data,
    beforeSend: function(){
      button.addClass('disabled');
      button.text(localization[language].sending+'...');
    },
    success: function(response){
      if (response == "TRUE")
      {
        form.parent('.feedback_form').remove();
        $('#success_sent').show();
      }
      else if(response == 'error')
      {
        button.text(localization[language].error);
        button.addClass('error');
      }
      else
      {
        button.text(localization[language].send);
        button.removeClass('disabled');
        $('<label class="error">'+response+'</label>').insertAfter(button);
      }
    }
  });
